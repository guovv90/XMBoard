...
public:
  ...
  //void sendintstr2(char* destId,char *nameditu,char *lati,char *lon);
  void sendintstr2(const char* destId,String nameditu,String lati,String lon);
  ...
  void SendMessage_PDU(const unsigned int number,char *addressee,char *message);//PDU模式发送信息
  bool DoCmdOk_meg(char *data,char *keyword);
  String Rx_meg(void);                                //接收短信控制
  void Rx_meg_int(void);                              //接收短信控制初始化
  String recvmeg(void);
  String Recv_number(void);            
  String RX_meg = "";                                 //解析的信息
  String RX_number = "";                                 //解析的信息

private:
	bool edp_connect;                                   //连接标志
